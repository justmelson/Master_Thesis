...
current_directory = os.getcwd()
final_directory = os.path.join(current_directory, r'Results/Exp5/')
if not os.path.exists(final_directory):
   os.makedirs(final_directory)
...
Cf_solar      = pd.read_pickle("import_data/cf_solar3h2d.pkl")
Cf_onshore    = pd.read_pickle("import_data/cf_onshore3h2d.pkl")
Cf_offshore   = pd.read_pickle("import_data/cf_offshore3h2d.pkl")
...
print("CC installed capacity in tCO2/h =","%.5f"% CC_installed) 
...
fixed_cost_no_r = pd.DataFrame(0.,index=years,columns=techs+storage_techs)
...
for year in years:
        for tech in techs:
            ...
            fixed_cost_no_r.at[year,tech] = model.fixed_costs[tech,year].value # EUR/MW/a
        ...
        for tech in storage_techs:
            ...
            fixed_cost_no_r.at[year,tech] = model.fixed_costs_storage[tech,year].value 
...
# fig.savefig("Results/Exp5/figures/{}-dispatch4days.png".format(filename),transparent=True,bbox_inches='tight')
...
ax.legend(bbox_to_anchor=(1, 1.05), ncol=1, fancybox=False, shadow=False)
ax.legend().set_visible(lgnd)
fig.savefig("Results/Exp5/figures/{}-capacity.png".format(filename),transparent=True,bbox_inches='tight')
...
ax.legend(bbox_to_anchor=(1, 1.05), ncol=1, fancybox=False, shadow=False)
ax.legend().set_visible(lgnd)
# fig.savefig("Results/Exp5/figures/{}-new_capacity.png".format(filename),transparent=True,bbox_inches='tight')
...
fixed_cost_no_r.plot(ax=ax2,linewidth=3,color=colors,ls='dashdot')
ax.set_xlabel("year")
...
ax.text(1, 0.4, textstr, transform=ax.transAxes, fontsize=12,
        verticalalignment='top', bbox=props)
ax.legend(bbox_to_anchor=(1, 1.05), ncol=1, fancybox=False, shadow=False)
fig.savefig("Results/Exp5/figures/{}-annuity.pdf".format(filename),transparent=True,bbox_inches='tight')
...
# ax.legend(bbox_to_anchor=(1, 1.05), ncol=1, fancybox=False, shadow=False)
# ax.legend().set_visible(lgnd)
# fig.savefig("Results/Exp5/figures/{}-SOC.png".format(filename),transparent=True)
...
# ax.legend(bbox_to_anchor=(1, 1.05), ncol=1, fancybox=False, shadow=False)
# ax.legend().set_visible(lgnd)
# fig.savefig("Results/Exp5/figures/{}-SOCallyears.png".format(filename),transparent=True)
...
# capex_onwind.to_pickle("Results/Exp5/capex_onwind{}.pkl".format(filename))
# capex_offwind.to_pickle("Results/Exp5/capex_offwind{}.pkl".format(filename))
# capex_battery.to_pickle("Results/Exp5/capex_battery{}.pkl".format(filename))
# capex_coal.to_pickle("Results/Exp5/capex_coal{}.pkl".format(filename))
# capex_nuclear.to_pickle("Results/Exp5/capex_nuclear{}.pkl".format(filename))
# capex_ocgt.to_pickle("Results/Exp5/capex_ocgt{}.pkl".format(filename))
