...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar 15 13:29:46 2022
...
model.objective = Objective(expr=constant +
                            ...
                            sum(model.generators_dispatch[tech,2020,hour] * parameters.at['marginal cost',tech]*1000*365*3 / 1e9 /(1+r)**(2020-years[0])
                                for hour in hours
                                for tech in techs)+
                            sum(model.generators_dispatch[tech,2023,hour] * parameters.at['marginal cost',tech]*1000*365*3 / 1e9 /(1+r)**(2023-years[0])
                                for hour in hours
                                for tech in techs)+
                            sum(model.generators_dispatch[tech,2026,hour] * parameters.at['marginal cost',tech]*1000*365*3 / 1e9 /(1+r)**(2026-years[0])
                                for hour in hours
                                for tech in techs)+
                            sum(model.generators_dispatch[tech,2029,hour] * parameters.at['marginal cost',tech]*1000*365*3 / 1e9 /(1+r)**(2029-years[0])
                                for hour in hours
                                for tech in techs)+
                            sum(model.generators_dispatch[tech,2032,hour] * parameters.at['marginal cost',tech]*1000*365*3 / 1e9 /(1+r)**(2032-years[0])
                                for hour in hours
                                for tech in techs)+
                            sum(model.generators_dispatch[tech,2035,hour] * parameters.at['marginal cost',tech]*1000*365*3 / 1e9 /(1+r)**(2035-years[0])
                                for hour in hours
                                for tech in techs)+
                            sum(model.generators_dispatch[tech,2038,hour] * parameters.at['marginal cost',tech]*1000*365*3 / 1e9 /(1+r)**(2038-years[0])
                                for hour in hours
                                for tech in techs)+
                            sum(model.generators_dispatch[tech,2041,hour] * parameters.at['marginal cost',tech]*1000*365*3 / 1e9 /(1+r)**(2041-years[0])
                                for hour in hours
                                for tech in techs)+
                            sum(model.generators_dispatch[tech,2044,hour] * parameters.at['marginal cost',tech]*1000*365*3 / 1e9 /(1+r)**(2044-years[0])
                                for hour in hours
                                for tech in techs)+ 
                            sum(model.generators_dispatch[tech,2047,hour] * parameters.at['marginal cost',tech]*1000*365*3 / 1e9 /(1+r)**(2047-years[0])
                                for hour in hours
                                for tech in techs)+
                            sum(model.generators_dispatch[tech,2050,hour] * parameters.at['marginal cost',tech]*1000*365*3 / 1e9 /(1+r)**(2050-years[0])
                                for hour in hours
                                for tech in techs))
...
def onshore_constraint2020(model,year,hour):
    return model.generators_dispatch["onshore_wind",2020,hour] <= model.generators["onshore_wind",2020]*Cf_onshore.at[hour,"Cf1"]
...
def onshore_constraint2023(model,year,hour):
    return model.generators_dispatch["onshore_wind",2023,hour] <= model.generators["onshore_wind",2023]*Cf_onshore.at[hour,"Cf1"]
...
def onshore_constraint2026(model,year,hour):
    return model.generators_dispatch["onshore_wind",2026,hour] <= model.generators["onshore_wind",2026]*Cf_onshore.at[hour,"Cf1"]
...
def onshore_constraint2029(model,year,hour):
    return model.generators_dispatch["onshore_wind",2029,hour] <= model.generators["onshore_wind",2029]*Cf_onshore.at[hour,"Cf1"]
...
def onshore_constraint2032(model,year,hour):
    return model.generators_dispatch["onshore_wind",2032,hour] <= model.generators["onshore_wind",2032]*Cf_onshore.at[hour,"Cf1"]
...
def onshore_constraint2035(model,year,hour):
    return model.generators_dispatch["onshore_wind",2035,hour] <= model.generators["onshore_wind",2035]*Cf_onshore.at[hour,"Cf1"]
...
def onshore_constraint2038(model,year,hour):
    return model.generators_dispatch["onshore_wind",2038,hour] <= model.generators["onshore_wind",2038]*Cf_onshore.at[hour,"Cf1"]
...
def onshore_constraint2041(model,year,hour):
    return model.generators_dispatch["onshore_wind",2041,hour] <= model.generators["onshore_wind",2041]*Cf_onshore.at[hour,"Cf1"]
...
def onshore_constraint2044(model,year,hour):
    return model.generators_dispatch["onshore_wind",2044,hour] <= model.generators["onshore_wind",2044]*Cf_onshore.at[hour,"Cf1"]
...
def onshore_constraint2047(model,year,hour):
    return model.generators_dispatch["onshore_wind",2047,hour] <= model.generators["onshore_wind",2047]*Cf_onshore.at[hour,"Cf1"]
...
def onshore_constraint2050(model,year,hour):
    return model.generators_dispatch["onshore_wind",2050,hour] <= model.generators["onshore_wind",2050]*Cf_onshore.at[hour,"Cf1"]
...
def offshore_constraint2020(model,year,hour):
    return model.generators_dispatch["offshore_wind",2020,hour] <= model.generators["offshore_wind",2020]*Cf_offshore.at[hour,"Cf1"]
...
def offshore_constraint2023(model,year,hour):
    return model.generators_dispatch["offshore_wind",2023,hour] <= model.generators["offshore_wind",2023]*Cf_offshore.at[hour,"Cf1"]
...
def offshore_constraint2026(model,year,hour):
    return model.generators_dispatch["offshore_wind",2026,hour] <= model.generators["offshore_wind",2026]*Cf_offshore.at[hour,"Cf1"]
...
def offshore_constraint2029(model,year,hour):
    return model.generators_dispatch["offshore_wind",2029,hour] <= model.generators["offshore_wind",2029]*Cf_offshore.at[hour,"Cf1"]
...
def offshore_constraint2032(model,year,hour):
    return model.generators_dispatch["offshore_wind",2032,hour] <= model.generators["offshore_wind",2032]*Cf_offshore.at[hour,"Cf1"]
...
def offshore_constraint2035(model,year,hour):
    return model.generators_dispatch["offshore_wind",2035,hour] <= model.generators["offshore_wind",2035]*Cf_offshore.at[hour,"Cf1"]
...
def offshore_constraint2038(model,year,hour):
    return model.generators_dispatch["offshore_wind",2038,hour] <= model.generators["offshore_wind",2038]*Cf_offshore.at[hour,"Cf1"]
...
def offshore_constraint2041(model,year,hour):
    return model.generators_dispatch["offshore_wind",2041,hour] <= model.generators["offshore_wind",2041]*Cf_offshore.at[hour,"Cf1"]
...
def offshore_constraint2044(model,year,hour):
    return model.generators_dispatch["offshore_wind",2044,hour] <= model.generators["offshore_wind",2044]*Cf_offshore.at[hour,"Cf1"]
...
def offshore_constraint2047(model,year,hour):
    return model.generators_dispatch["offshore_wind",2047,hour] <= model.generators["offshore_wind",2047]*Cf_offshore.at[hour,"Cf1"]
...
def offshore_constraint2050(model,year,hour):
    return model.generators_dispatch["offshore_wind",2050,hour] <= model.generators["offshore_wind",2050]*Cf_offshore.at[hour,"Cf1"]
...
def co2_constraint(model,tech,year,hour):
    return co2_budget >= sum((model.generators_dispatch[tech,year,hour] * 365 * parameters.at["specific emissions",tech]) for tech in techs for hour in hours for year in years)
...
fig,ax = plt.subplots()
fig.set_dpi(400)
Cf_onshore["Cf1"].plot(ax=ax,linewidth=3,label='Onshore wind')
Cf_solar["Cf1"].plot(ax=ax,linewidth=3,label='Solar PV')
Cf_offshore["Cf1"].plot(ax=ax,linewidth=3,label='Offshore wind')
ax.legend(bbox_to_anchor=(1, 1), ncol=1, fancybox=False, shadow=False)
ax.set_ylim([0,0.3])
ax.set_xlabel("hour")
...
fig,ax = plt.subplots()
fig.set_dpi(400)
Cf_onshore["Cf10"].plot(ax=ax,linewidth=3,label='Onshore wind')
Cf_solar["Cf10"].plot(ax=ax,linewidth=3,label='Solar PV')
Cf_offshore["Cf10"].plot(ax=ax,linewidth=3,label='Offshore wind')
ax.legend(bbox_to_anchor=(1, 1), ncol=1, fancybox=False, shadow=False)
ax.set_ylim([0,0.5])
ax.set_xlabel("hour")
ax.set_ylabel("Capacity factor")
