...
def capacity_constraint(model,tech,year):
    return sum(model.generators[tech,year] for tech in techs) <= 10000
...
def co2_constraint(model,tech,year,hour): # MtCO2 = MWh*MtCO2/MWh
    return co2_budget >= sum((model.generators_dispatch[tech,year,hour] * 1000*dty*interval * parameters.at["specific emissions",tech]) for tech in techs for hour in hours for year in years) if "co2-0p2" in scenario else Constraint.Skip
